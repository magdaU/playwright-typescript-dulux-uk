...
test.describe('Cart quantity boundary values', { tag: ['@cart', '@regression', '@desktop'] }, () => {
  const colourFamily = 'Violet';
  const shade = 'Sugared Lilac';
  const productName = 'Dulux Paint Mixing Easycare Washable & Tough Matt';

  test('quantity input rejects zero, negative and above-max values', async ({
    page,
    homePage,
    navigation,
    colorSelectionPage,
    ...
    // Uses a regular paint product rather than the colour tester: the tester is currently
    // unorderable site-wide (see BUG-005 in BUG_REPORTS.md), but the quantity input's
    // validation behaviour doesn't depend on which product is in the basket.
    await cartPage.open();
    ...
    await test.step('a quantity above the input max (999) is rejected and surfaces an error', async () => {
      ...
      await expect(cartPage.getQuantity()).toHaveValue('1');
      await expect(page.getByText('Sorry we encountered an error, please try again.')).toBeVisible();
    });
  });
});
